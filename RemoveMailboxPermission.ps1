...
Param(
  ...
  [Parameter(Position=1, Mandatory=$false, ValueFromPipeline=$true)][string] $RemoveAccessTo
)
...
if (([string]::IsNullOrEmpty($SourceMailbox) -eq $true) -or ([string]::IsNullOrEmpty($RemoveAccessTo) -eq $true))
{
  ...
  Write-Host "-------------------------------------------------------------------------------------------------";  "";
  $SourceMailbox = Read-Host "Casella da modificare (esempio: sharedmailbox@contoso.com)            "
  $RemoveAccessTo = Read-Host "Utente al quale togliere l'accesso (esempio: mario.rossi@contoso.com) "
}
