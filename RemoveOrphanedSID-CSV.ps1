...
Param(
  ...
  [Parameter(Position=1, Mandatory=$false, ValueFromPipeline=$true)][string] $Mbox,
  [Parameter(Position=2, Mandatory=$false)][switch] $Remove
)
...
Function Main {
  ...
  if ([string]::IsNullOrEmpty($CSV)) {
    $ExportList = "C:\temp\OrphanedSID.csv"
    } else {
      $ExportList = $CSV
    }
