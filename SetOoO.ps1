...
Param(
  ...
  [Parameter(Position=1, Mandatory=$false, ValueFromPipeline=$true)][string] $Status,
  [Parameter(Position=2, Mandatory=$false, ValueFromPipeline=$true)][string] $Int,
  [Parameter(Position=3, Mandatory=$false, ValueFromPipeline=$true)][string] $Ext,
  [Parameter(Position=4, Mandatory=$false, ValueFromPipeline=$true)][string] $Start,
  [Parameter(Position=5, Mandatory=$false, ValueFromPipeline=$true)][string] $End
)
...
if ( [string]::IsNullOrEmpty($Int) -and [string]::IsNullOrEmpty($Ext) ) {
  $Int = "Sono assente per ferie e avrà limitato accesso alla mia mailbox.<br />
  Risponderà alla vostra comunicazione appena possibile.
  <br /><br />
  Buona giornata."
}
