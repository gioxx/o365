...
Param( 
    [Parameter(Position=0, Mandatory=$true, HelpMessage="Utente da modificare (es. mario.rossi)", ValueFromPipeline=$true)] 
    [string] $User,
	[Parameter(Position=1, Mandatory=$true, HelpMessage="Alias da aggiungere (es. mario.rossi.alias@contoso.com)", ValueFromPipeline=$true)] 
    [string] $Alias,
    [switch] $Remove
)
